...
export default function SalesPage() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const [salesData, khatasData] = await Promise.all([
        salesAPI.getAll(),
        khatasAPI.getAll(),
      ])
      setSales(salesData)
  ...
  const getKhataTotals = () => {
    ...
    khatasSales.forEach((sale) => {
      totals[sale.currency] = (totals[sale.currency] || 0) + sale.amount
    })
    return totals
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // For new sales, require an active khata
    if (!editingSale && !activeKhata) {
    ...
    const targetKhata = editingSale 
      ? khatas.find(k => k.id === editingSale.khataId) 
      : activeKhata
    ...
    if (targetKhata) {
      ...
      if (saleDate < khataStartDate || saleDate > khataEndDate) {
        showToast(
          `Sale date must be between ${formatDisplayDate(targetKhata.startDate)} and ${formatDisplayDate(targetKhata.endDate)}`,
          "error"
        )
        return
      }
    ...
    const saleData: Sale = {
      ...
      date: formData.date,
      // Keep the original khataId when editing, use activeKhata for new sales
      khataId: editingSale ? editingSale.khataId : (activeKhata?.id || ""),
    }
  ...
  const handleCreateKhata = async (e: React.FormEvent) => {
    ...
    try {
      const newKhata = await khatasAPI.create(khataForm)
      showToast("Khata created successfully! Previous Khata has been closed.", "success")
      ...
      showToast("Failed to create Khata", "error")
      console.error("Error creating khata:", error)
    }
  }

  const handleDeleteKhataClick = (khata: Khata) => {
    // Check if there are any sales associated with this khata
    const khataSales = sales.filter((sale) => sale.khataId === khata.id)
    
    if (khataSales.length > 0) {
      showToast("Please first delete all sales in this Khata, then you can delete the Khata", "error")
      return
    }
    
    // If no sales, show confirmation modal
  ...
  const handleDeleteKhata = async () => {
    ...
    try {
      await khatasAPI.delete(deleteKhataModal.khata.id)
      ...
      <div className="space-y-6 px-4 sm:px-6 lg:px-0">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-gray-600">Manage your sales records by Khata</p>
          </div>
        ...
        {activeKhata ? (
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <p className="text-yellow-800 text-sm font-medium">
              No active Khata. Click "New Khata" to create one before adding sales.
            </p>
          </div>
        )}
          ...
          <div className="bg-white rounded-lg shadow p-4 sm:p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {(["USD", "PKR", "SAR", "CNY"] as const).map((cur) => (
                <div key={cur} className="bg-blue-50 rounded-lg p-3 text-center">
                  <p className="text-xs font-medium text-blue-600">{cur}</p>
        ...
        {selectedKhata ? (
          <div className="bg-white rounded-lg shadow">
            <div className="p-4 sm:p-6 border-b">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-semibold text-gray-900">
                  {selectedKhata.name} — Sales Records
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={7} className="px-6 py-8 text-center text-gray-500 text-sm">
                        No sales records in this Khata
                      </td>
                    </tr>
                  ) : (
                    currentItems.map((sale) => (
                      <tr key={sale.id} className="hover:bg-gray-50">
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-gray-500 text-sm">{formatDisplayDate(sale.date)}</td>
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap font-medium text-gray-900 text-sm">{sale.customer}</td>
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-gray-500 text-sm">{sale.item}</td>
                        <td className="px-4 sm:px-6 py-4 text-gray-500 text-sm">{sale.description}</td>
                        ...
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-gray-500 text-sm">{sale.currency}</td>
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                          <div className="flex space-x-2">
                            <button onClick={() => handleEdit(sale)} className="p-1 text-blue-600 hover:text-blue-800">
                              <Edit className="h-4 w-4" />
                            </button>
                            <button onClick={() => setDeleteModal({ isOpen: true, sale })} className="p-1 text-red-600 hover:text-red-800">
          ...
          <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
            No Khata selected. Create a new Khata to start adding sales.
          </div>
        )}
        ...
        {isDialogOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex !mt-0 items-center justify-center z-50 px-4">
            <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-md">
              ...
              {(() => {
                // Show the khata that the sale belongs to when editing, otherwise show active khata
                const displayKhata = editingSale 
                  ? khatas.find(k => k.id === editingSale.khataId)
                  : activeKhata
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end space-x-2 pt-4">
                  ...
                  <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm">{editingSale ? "Update" : "Add"} Sale</button>
                </div>
              </form>
            </div>
          </div>
