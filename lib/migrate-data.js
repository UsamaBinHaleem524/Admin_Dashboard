...
// Migration function to add totalAmount field to existing invoices
const migrateInvoicesTotalAmount = async () => {
  console.log('Starting migration to add totalAmount field to invoices...');
  
  try {
    const invoicesResponse = await fetch('/api/invoices');
    if (invoicesResponse.ok) {
      const invoices = await invoicesResponse.json();
      console.log(`Found ${invoices.length} invoice records to migrate`);
      ...
      for (const invoice of invoices) {
        if (!invoice.totalAmount) {
          ...
          if (updateResponse.ok) {
            console.log(`✅ Updated invoice ${invoice.id} with totalAmount: ${totalAmount}`);
          } else {
            console.error(`❌ Failed to update invoice ${invoice.id}`);
          }
        }
      }
    }
  } catch (error) {
    console.error('❌ Error migrating invoices totalAmount:', error);
  }
};
...
if (typeof window !== 'undefined') {
  ...
  window.migrateInvoicesTotalAmount = migrateInvoicesTotalAmount;
...
export { migrateLocalStorageToMongoDB, migrateSalesAndPurchasesFields, migrateInvoicesTotalAmount, migrateCustomerTransactionsToNewFormat, migrateSupplierTransactionsToNewFormat }; 